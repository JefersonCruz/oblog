...
if( $pid != "" && !is_search() ) {
	...
	/* Sidebar Layout */
	if( get_post_meta( $pid, 'maxelectric_cf_sidebar_owlayout', true ) != "" ) {
	...
	/* Widget Area */
	if( get_post_meta( $pid, 'maxelectric_cf_widget_area', true ) != "" ) {
		$widgetarea = get_post_meta( $pid, 'maxelectric_cf_widget_area', true );
	}
	else {
